...
public:
  ...
  [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
    if(document_id < 0 || documents_.count(document_id) || ContainSpecialSymbols(document)) {
      return false;
    }
    ...
    documents_sequence_.push_back(document_id);
    return true;
  }
  ...
  [[nodiscard]] bool FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate, vector<Document>& result) const {
    if (ContainSpecialSymbols(raw_query)) {
      return false;
    };
  ...
  int GetDocumentId(int document_number) const {
    if (document_number < 0 || document_number >= documents_.size()) {
      return SearchServer::INVALID_DOCUMENT_ID;
    }
    return documents_sequence_.at(document_number);
  }
...
private:
  ...
  vector<int> documents_sequence_;
  ...
  static bool ContainSpecialSymbols(const string& s) {
    auto iter = find_if(s.begin(), s.end(), [](const char c) {
      int ascii = (int) c;
      return ascii >= 0 && ascii <= 31;
    });
    return iter != s.end();
  }
