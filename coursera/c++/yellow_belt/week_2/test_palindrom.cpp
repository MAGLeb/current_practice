...
void TestEmptyString() {
  ...
  Assert(IsPalindrome("q"), "One sign");
  Assert(!IsPalindrome("qe"), "Two letters");
}
...
void TestBaseCases() {
  ...
  Assert(!IsPalindrome("leqwvqwqel"), "Stop before middle 1");
  Assert(!IsPalindrome("leqwveqel"), "Stop before middle 2");

  Assert(!IsPalindrome("ZweY_YewX"), "ZweY_YewX");

  Assert(!IsPalindrome("leVellevel"), "Big letter");
}

int main() {
  TestRunner runner;
  runner.RunTest(TestEmptyString, "TestEmptyString");
  runner.RunTest(TestBaseCases, "TestBaseCases");
  return 0;
}