...
pair<Date, string> Entry(const Date &date, const string &event) {
  return make_pair(date, event);
}
...
void TestsMyCustom() {
...

  {
    ...
    AssertEqual(1, DoRemove(db, R"(event == "c" OR event == "d")"),
                "My test 3");
    AssertEqual(1, DoRemove(db, R"(event == "e" AND event != "a")"),
                "My test 4");
    db.Add({2019, 11, 30}, "a");
    AssertEqual("2019-12-03 f\n1", DoFind(db, R"(date >= 2019-12-3)"),
                "My test 5");
    AssertEqual(Entry({2019, 12, 3}, "f"), db.Last({2019, 12, 4}),
                " My test 6");
    ...
    AssertEqual(Entry({2019, 12, 2}, "c c"), db.Last({2019, 12, 2}),
                " My test 7");
    ...
    db.Add({2019, 12, 3}, "m");
    AssertEqual(Entry({2019, 12, 3}, "m"), db.Last({2019, 12, 3}),
                " My test 9");
    ...
    AssertEqual(
        ...
        DoRemove(
            db,
            R"(event == "e" AND event != "a" OR event == "m" AND date == 2019-12-3)"),
        "My test 10");
...

  {
    ...
    AssertEqual(Entry({2019, 12, 1}, "c"), db.Last({2019, 12, 1}),
                " My test 20");
...
void TestDatabase() {
  ...
  auto empty_predicate = [empty_condition](const Date &date,
                                           const string &event) {
...
void TestLogicalOperationNode() {
  {
    LogicalOperationNode lon(LogicalOperation::And, make_shared<EmptyNode>(),
                             make_shared<EmptyNode>());
  ...
  {
    LogicalOperationNode lon(LogicalOperation::Or, make_shared<EmptyNode>(),
                             make_shared<EmptyNode>());
